...
MAX_SUPERVISED_ROWS = 300_000     # lấy 300k session để train cho nhẹ
...
```python
# Lấy 300k session, stratify=y để giữ đúng tỉ lệ ~3,5% anomaly
...
Kết quả kiểm tra in ra: **0 visitor trùng** giữa train/validation/test. Sau đó dùng `RobustScaler` (chuẩn hóa chịu được outlier) fit trên train rồi áp cho cả 3 tập — fit chỉ trên train để không rò rỉ thông tin test.
...
### Tầng 2 — Lấy mẫu phân tầng (stratified sampling)

Khi rút 300.000 session để train, dùng `stratify=y` để **giữ đúng tỉ lệ ~3,5% anomaly** ở mọi tập, tránh tập nào bị mất sạch lớp hiếm:
...
Notebook in ra tỉ lệ anomaly của từng tập để kiểm chứng chúng xấp xỉ nhau.

### Tầng 3 — Tinh chỉnh ngưỡng quyết định (threshold tuning)

Không cắt mặc định 0,5. Thay vào đó quét ngưỡng tối ưu **F1 trên validation** rồi cố định áp lên test. Vì lớp dương hiếm, ngưỡng tối ưu thường khác 0,5 (XGBoost = **0,869**). Đây là cách bù mất cân bằng ở khâu ra quyết định (xem hàm `choose_best_threshold` ở Phần 8).
...
threshold = choose_best_threshold(y_val, val_score, 'XGBoost')     # chốt ngưỡng = 0,869 trên val
...
- **XGBoost & LightGBM dẫn đầu**, recall ~0,92–0,93 (bắt được phần lớn anomaly), precision ~0,81–0,82.
...
|---|--:|--:|--:|--:|--:|
| Validation | 0,814 | 0,929 | 0,868 | 0,928 | 0,869 |
| Test | 0,812 | 0,918 | 0,862 | 0,922 | 0,869 |

Khoảng cách val–test rất nhỏ (F1 0,868 → 0,862) → **ổn định, chưa overfit**.

### Ma trận nhầm lẫn XGBoost (test, 60.000 session)
...
Bắt đúng 1.942 / bỏ sót 174 (recall 0,918); báo nhầm 450 (precision 0,812). (Suy ra từ support=2.116, predicted=2.392; khớp accuracy 0,9896.)

### Đặc trưng quan trọng nhất (feature importance)

Dẫn đầu là nhóm thời gian/nhịp: `std_interval_sec`, `max_interval_sec`, `session_duration_sec` — hợp lý vì bot/rapid-fire có nhịp thao tác đều/nhanh bất thường so với người thật.
...
→ XGBoost test F1 = **0,994**, PR-AUC ≈ 1,0. Gần tuyệt đối vì mô hình **chép lại luật** → đây là minh chứng leakage, **không dùng làm kết luận**.
...
→ F1 tụt còn **0,209**, PR-AUC 0,092. Đúng kỳ vọng (mô hình không thể học từ nhãn rác) → xác nhận pipeline không có rò rỉ ẩn. Notebook còn tự cảnh báo nếu PR-AUC shuffle cao bất thường.
...
2. *F1 safe vẫn cao (0,86) — hết leakage chưa?* → đã loại biến trực tiếp tạo luật; các biến thời gian còn lại vẫn tương quan gián tiếp nên còn một phần tín hiệu. Khoảng cách full (0,99) → safe (0,86) cho thấy đã giảm leakage đáng kể nhưng chưa triệt để.
3. *LightGBM cao hơn XGBoost (0,873 > 0,862)* → cùng họ boosting, sát nhau, củng cố độ tin cậy; chọn XGBoost làm chính theo phân công.
